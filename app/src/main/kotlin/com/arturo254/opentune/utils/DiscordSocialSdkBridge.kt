...
import com.arturo254.opentune.utils.DiscordSocialSdkBridge.createClient
import com.arturo254.opentune.utils.DiscordSocialSdkBridge.nativeRunCallbacks
...
object DiscordSocialSdkBridge {
    ...
    private var onStatusChangedListener: ((status: Int, error: Int, errorDetail: Int) -> Unit)? =
        null
    ...
    fun connect() {
        if (isAvailable) nativeConnect()
    }

    fun disconnect() {
        if (isAvailable) nativeDisconnect()
    }
    ...
    fun clearRichPresence() {
        if (isAvailable) nativeClearRichPresence()
    }
